...
from self_judged_refusal_direction.editing import WeightEditPlan, WeightEditPlanBuilder
...
def test_temporary_and_permanent_logits_match(tied: bool, direction: torch.Tensor) -> None:
    ...
    baseline_logits = temporary_model(token_ids, multimodal)

    with plan.temporary(temporary_model):
        temporary_logits = temporary_model(token_ids, multimodal)
    restored_logits = temporary_model(token_ids, multimodal)
    ...
    if tied:
        assert permanent_model.embedding.weight is permanent_model.lm_head.weight


def test_projected_outputs_are_orthogonal(direction: torch.Tensor) -> None:
    torch.manual_seed(4)
...
def test_temporary_intervention_cleans_up_after_error(direction: torch.Tensor) -> None:
    torch.manual_seed(8)
    model = ToyModel(tied=True)
    plan = build_plan(model, direction)
    token_ids = torch.tensor([[1, 3, 5]])
    multimodal = torch.randn(1, 3, 4)
    baseline_logits = model(token_ids, multimodal)

    with pytest.raises(RuntimeError, match="stop"), plan.temporary(model):
        model(token_ids, multimodal)
        raise RuntimeError("stop")

    torch.testing.assert_close(model(token_ids, multimodal), baseline_logits)
